...
impl Conn {
    pub async fn incoming(self_id: (Gid, Pid), sock: TcpStream) -> Result<Self, std::io::Error> {
        ...
        tx.send(Message::Handshake { gid: self_id.0, pid: self_id.1 }).await?;
        if let Message::Handshake { gid, pid } = rx.next().await.unwrap_or(Err(std::io::ErrorKind::ConnectionReset.into()))? {
    ...
    pub async fn request(from: (Gid, Pid), to: (Gid, Pid), addr: SocketAddr, req: Message) -> Result<Self, std::io::Error> {
        let sock = TcpStream::connect(addr).await?;
        sock.set_nodelay(true).unwrap();
        let (mut rx, mut tx) = bincode_split::<Message, Message, _>(sock);
        tx.feed(Message::Handshake { gid: from.0, pid: from.1}).await?;
        tx.send(req).await?;
        if let Message::Handshake { gid, pid } = rx.next().await.unwrap_or(Err(std::io::ErrorKind::ConnectionReset.into()))? {
            if (gid, pid) == to {
    ...
    pub async fn recv(&mut self) -> Result<Message, std::io::Error> {
        Ok(self.rx.next().await.unwrap_or(Err(std::io::ErrorKind::ConnectionReset.into()))?)
    }

    pub fn split(self) -> (ConnWrite, ConnRead) {
        let w = ConnWrite { to: self.to, tx: self.tx };
        let r = ConnRead { to: self.to, rx: self.rx };
        (w, r)
    }
}
